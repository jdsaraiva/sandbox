...
/*
  ...
  Description: Feetches users info from Twitter API
  Author: Dessain Saraiva - Original version at https://github.com/timwhitlock/wp-twitter-api
  */
...
function latest_tweets_render( $screen_name, $count, $rts, $ats, $pop = 0 ){
    ...
    try {
        ...
        foreach( $tweets as $tweet ){
            ...
            // show tweets according to logged in user
            $handle = 'WebSummit';
 ...
 */
function latest_tweets_render_html( $screen_name = 'WebSummit', $num = 5, $rts = true, $ats = true, $pop = 0 ){
...
add_shortcode( 'user_info', 'user_info_func' );
...
if( is_admin() ){
    ...
    function latest_tweets_plugin_row_meta( $links, $file ){
        if( false !== strpos($file,'/twitter-api-feed.php') ){
